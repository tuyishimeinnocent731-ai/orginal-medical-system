...
const Payroll: React.FC = () => {
  const getStatusColor = (status: PayrollEntry['status']) => {
    return status === 'Paid' ? 'bg-green-100 text-green-800 dark:bg-green-900/50 dark:text-green-200' : 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/50 dark:text-yellow-200';
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 w-full sm:w-auto">Run Payroll Cycle</button>
      </div>
        ...
        <table className="w-full text-left">
          <thead className="border-b dark:border-gray-700">
            <tr>
              ...
              <th className="p-3 hidden sm:table-cell">Pay Period</th>
              <th className="p-3 hidden md:table-cell">Gross Pay</th>
          ...
          <tbody>
            {mockPayroll.map(entry => (
              <tr key={entry.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                ...
                <td className="p-3 hidden sm:table-cell">{entry.period}</td>
                <td className="p-3 hidden md:table-cell">${entry.grossPay.toFixed(2)}</td>
